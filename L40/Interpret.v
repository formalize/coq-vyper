...
From Vyper.L40 Require Import AST Callset Descend Stmt.
...
Local Lemma interpret_call_helper {this_decl: decl}
                                  ...
                                  {arity: N}
                                  {body: block}
                                  (E: this_decl = FunDecl fun_name arity body):
  let _ := string_set_impl in FSet.is_subset (block_callset body) (decl_callset this_decl) = true.
Proof.
...
:= let _ := memory_impl in
   ...
   | S new_call_depth_bound => fun Ebound =>
        ...
        | FunDecl _ arity body => fun E =>
            if (List.length arg_values =? N.to_nat arity)%nat
              then let loc := OpenArray.from_list arg_values in
                   let '(world', loc', result) := interpret_block Ebound fc (interpret_call builtins)
